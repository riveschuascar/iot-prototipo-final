...
const char* MQTT_BROKER = "a1n3h8klbo6gqj-ats.iot.amazonaws.com";
...
void reportStates() {
  ...
  GateServo::GateState gateState = gate.getState();
  outputDoc["state"]["reported"]["gate_state"] = (gateState == GateServo::open) ? "open" : "closed";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == UPDATE_DELTA_TOPIC) {
    ...
    if (state.containsKey("gate_state")) {
      ...
      GateServo::GateState currentValve = gate.getState();
      
      // Comparar con strings
      if (desiredValve == "open" && currentValve != GateServo::open) {
        gate.openGate();
        reportStates();
      }
      else if (desiredValve == "closed" && currentValve != GateServo::closed) {
        gate.closeGate();
        reportStates();
      }
    }
